...
class _JustAudioBackgroundPlugin extends JustAudioPlatform {
  ...
  /// WetOwl: players beyond the first, which are somebody else's — the booth's two
  /// decks. They are made on the real platform and get no media session: there is
  /// one notification, one lockscreen and one set of headset buttons, and they
  /// belong to the player the app is built around. See WETOWL.md.
  final _passedThrough = <String>{};

  @override
  Future<AudioPlayerPlatform> init(InitRequest request) async {
    if (_playerId != null) {
      // WetOwl: a second player is passed through rather than refused.
      _passedThrough.add(request.id);
      return _platform.init(request);
    }
  ...
  Future<DisposePlayerResponse> disposePlayer(
    ...
    // WetOwl: a passed-through player is the real platform's to let go of.
    if (_passedThrough.remove(request.id)) {
      return _platform.disposePlayer(request);
    }
  ...
  Future<DisposeAllPlayersResponse> disposeAllPlayers(
    ...
    // WetOwl: and everything passed through goes with them.
    if (_passedThrough.isNotEmpty) {
      _passedThrough.clear();
      await _platform.disposeAllPlayers(request);
    }
    final player = _player;
    _player = null;
    await player?.release();
