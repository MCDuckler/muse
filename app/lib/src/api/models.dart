...
/// A song Shazam recognised, and what it turned out to be here.
class Shazam {
  const Shazam({
    required this.id,
    required this.title,
  ...
  /// What it was matched to here, once anything has been.
  final Track? track;
  final double? confidence;

  /// Whether it has been looked for yet. Not the same as having been found: a tag
  /// nothing answered to has been looked at and has no track.
  final DateTime? lookedAt;

  bool get matched => track != null;
  bool get waiting => lookedAt == null;

  factory Shazam.fromJson(Map<String, dynamic> j) => Shazam(
        id: j['id'] as int,
        title: (j['title'] ?? '') as String,
        artist: j['artist'] as String?,
        taggedAt: j['tagged_at'] == null
            ? null
            : DateTime.tryParse(j['tagged_at'] as String)?.toLocal(),
        url: j['url'] as String?,
        confidence: (j['confidence'] as num?)?.toDouble(),
        lookedAt: j['looked_at'] == null
            ? null
            : DateTime.tryParse(j['looked_at'] as String)?.toLocal(),
        track: j['track'] == null
            ? null
            : Track.fromJson(j['track'] as Map<String, dynamic>),
      );
...
/// A library of them, and how far through looking them up the server has got.
class Shazams {
  const Shazams({
    required this.items,
    required this.total,
  ...
  final List<Shazam> items;
  final int total;
  final int matched;

  /// Not yet looked for. The screen says so rather than showing a list that quietly
  /// fills in while somebody watches it and wonders whether it is broken.
  final int waiting;

  factory Shazams.fromJson(Map<String, dynamic> j) => Shazams(
        items: [
          for (final s in (j['items'] ?? const []) as List)
            Shazam.fromJson(s as Map<String, dynamic>)
        ],
        total: (j['total'] ?? 0) as int,
        matched: (j['matched'] ?? 0) as int,
        waiting: (j['waiting'] ?? 0) as int,
      );
}
