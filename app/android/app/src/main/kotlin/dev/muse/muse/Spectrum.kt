...
class Spectrum(private val activity: Activity) : EventChannel.StreamHandler {
    ...
    /** How many bars the app draws. Small: this is decoration, not an instrument. */
    private val bands = 24
        ...
        try {
            visualizer = Visualizer(sessionId).apply {
                ...
                setDataCaptureListener(
                    ...
                    // Half the maximum rate: sixty frames a second of decoration is
                    // more than the eye needs and more than the battery should pay for.
    ...
    private fun bandsFrom(fft: ByteArray): DoubleArray {
        ...
        for (band in 0 until bands) {
            // Widening bands: the low end of the spectrum is where the detail is.
            val high = min(bins, (low * 1.32).toInt().coerceAtLeast(low + 1))
            var peak = 0.0
            ...
            val db = if (peak <= 0) 0.0 else 20 * log10(peak / 128.0) + 60
            out[band] = (db / 60.0).coerceIn(0.0, 1.0)
            low = high
            if (low >= bins) break
        }
        return out
